...
- B 树：B 树的树高远小于 AVL 树和红黑树，磁盘 IO 次数大大减少，且 B 树将相邻的数据存储在同一节点(空间局部性)，缓存的命中率更高
- B+ 树：B+ 树比 B 树具备更少的 IO 次数(直到叶节点才进行磁盘 IO)，更稳定的查询效率(查询必定到达叶节点)，更适于范围查询(叶节点保存指向兄弟节点的指针)
  ...
  - IO 次数：B+ 树的中间节点不存储真实数据，因此能存储更多记录，树的高度更低，IO 次数更少，且利用空间局部性使得 B+ 树的缓存命中率更高(节点存储更多记录)
  - 占用空间：B 树中的键只会出现一次；B+ 树中的键可能重复出现，因为在中间节点出现的键必定会在叶节点出现，占用的空间更大
...
- 避免使用 JOIN 关联太多表：JOIN 内部使用嵌套循环实现关联查询，效率较低
  - MySQL 会对关联操作分配关联缓存，若一个 SQL 中关联的表过多，可能导致分配过多关联缓存，占用大量服务器内存
  - 关联表也会产生临时表，影响查询效率
  - 优化：
    - 在业务层处理多个单表查询结果：对数据库进行多次单表查询，在业务层进行联表
    - 数据冗余：将重要的关联结果在表中做冗余，只有在表结构比较稳定的情况下才使用
