...
![](https://pic4.zhimg.com/80/v2-ff5ca8d9919221d99cb49784188ccb6f_720w.webp)

- MOVED：永久重定向，请求会一直发向新节点
  1. 客户端向计算出的 hash slot 发送请求
  2. 旧节点返回 moved 响应，并告知客户端新节点地址
  3. 客户端之后向新节点发送请求

![](https://pic3.zhimg.com/80/v2-5654bc0adfa4ea2d8be87eeb396e80a2_720w.webp)

#### 节点通信

基于 Gossip 协议的通信机制，节点间互相发送多种 Gossip 消息：

- MEET：向指定 Redis 节点发送一条 MEET 信息，用于将其添加进 Redis Cluster 成为新的 Redis 节点
- PING/PONG：生命包，集群中的节点会定时向其他节点发送 PING 信息，来交换各个节点状态信息，检查各个节点状态(在线，疑似下线 PFAIL，已下线 FAIL)
- FAIL：节点 A 发现节点 B PFAIL，且在下线报告的有效期间集群中半数以上的节点将 B 节点标记为 PFAIL，节点 A 会向集群广播一条 FAIL 消息，通知其他节点将故障节点 B 标记为 FAIL

Redis Cluster 相当于内置 Redis Sentinel，内部的各个 Redis 节点通过 Gossip 协议互相探测健康状态，故障时可以自动切换
...
没有索引的情况下，MongoDB 必须执行**集合扫描**，即扫描集合中的每个文档，已选择与查询语句匹配的文档

如果存在合适的索引，MongoDB 可以使用该索引来限制它必须检查的文档的数量，且可以使用索引中的排序返回排序后的结果

引入索引能显著缩短查询时间，使用和维护索引也会造成额外的开销，执行写操作时必须同步更新索引，故在写多读少的场景下不适用

#### 索引类型

- 单字段索引：建立在单个字段上的所有，索引创建的排序顺序没有影响，MongoDB 可以从头/尾开始遍历
- 复合索引：建立在多个字段上的索引
  - 索引字段顺序：最左前缀原则
- 多键索引：MongoDB 的一个字段可能是数组，由此字段创建索引时就是多键索引，MongoDB 会为数组的每个值创建索引，即以数组中的值为条件查询，也会走索引
- 哈希索引：以数据的哈希值索引，用于哈希分片集群
- 文本索引：支持对字符串内容的文本搜索查询，一个集合只能有一个文本搜索索引，但该索引能够覆盖多个字段，性能低下
- 地理位置索引：基于经纬度的索引，适用于 2D 和 3D 的位置查询
- 唯一索引：确保索引字段不会存储重复值
- TTL 索引：提供过期机制，允许为每个文档设置一个过期时间，文档过期后会被删除，只能用于单字段索引，且每个字段只能有一个 TTL 索引
  - MongoDB 会开启一个后台线程读取 TTL 索引的值判断文档是否过期，但不保证已过期的数据会立刻被删除(后台线程定期触发删除任务，删除量过大时会导致两次触发间隔未删除完毕)

### 高可用

#### 复制集群

复制集群/副本集群，一组维护相同数据集合的 mongod 进程

客户端连接到整个 MongoDB 复制集群，主节点复制写操作，从节点可以进行读操作(默认为主节点负责整个复制集群的读)

主节点发生故障时，自动将从节点中选择一个作为新的主节点，保证集群正常使用(客户端透明)

- 1 个主节点：整个集群的写操作入口，接收所有写操作，将集合所有变化记录到操作日志中(oplog)
  - oplog：类型 binlog，记录写操作的增量日志，用于主从同步
- n 个从节点：从主节点同步数据，作为主节点的备用节点，可以通过将优先级配置成 0 来禁止升级为主节点
- 1 个仲裁节点：为了节约资源或多机房容灾，只负责主节点选举时投票，不存储数据，保证能有节点获得多数赞成票

优点：

- 实现 failover：自动故障恢复，主节点失效时能自动将从节点升级，保证集群正常使用
- 读写分离

#### 分片集群

MongoDB 的分布式版本，分片集数据被均衡地分布在不同分片中，大幅提升了整个集群的数据容量上限，也将读写的压力分散到不同分片

组成部分：

- 配置服务器：本质上是一个 MongoDB 副本集，负责存储集群的各种元数据
- 路由服务：不存储具体数据，从配置服务器获取集群配置，将请求转发到特定的分片，且整合分片结果返回给客户端
- 分片：每个分片是整体数据的一部分子集，从 MongoDB 3.6 开始，每个分片必须部署为副本集架构

优点：利用分片集群实现较为廉价、灵活的水平扩展，增大数据存储量，支持更高吞吐量

##### 分片键

数据分区的前提，决定集合内的文档如何在集群的多个分片间的分布状况

分片键为文档中的一个特殊字段：

- 必须在所有文档中都出现
- 是集合的一个索引(单索引，复合索引的前缀索引)，不能是多索引、文本索引、地理空间位置索引
- 不能超过 512 字节

##### 分片策略

- 基于范围分片：按照分片键的值的范围将数据拆分为不同的块(Chunk)，每个块包含一段范围内的数据，当分片键的计数大、频率低、值非单调变更时，范围分片更高效
  - 可以快速定位请求数据并转发到相应的分片节点
  - 可能导致数据在分片节点上分布不均匀，造成读写热点，且不具备写分散性
  - 适用于分片键非单调变化、分片键的值基数大且重复频率低、需要范围查询等业务场景
- 基于 Hash 值分片：计算单个字段的 Hash 值作为索引值，并以 Hash 值的范围将数据拆分成不同的块
  - 数据分布更均匀，具备写分散性
  - 不适合范围查询，需要将读请求分发到所有分片节点
  - 适用于分片键单调变化、分片键的值基数大且重复频率低、需要写入的数据随机分发、数据读取随机性大等业务场景

##### 分片数据存储

Chunk 本质上由一组文档组成的逻辑数据单元，每个 Chunk 包含一定范围分片键的数据，互不相交且并集为全部数据

分片集群只记录 Chunk 在哪个分片上，以及 Chunk 包含哪些数据

当 Chunk 存储数据到达上限时，会触发 Chunk 分裂，Mongos 中的均衡器会执行自动平衡，尝试令各个分片上 Chunk 的数量保存均衡(再平衡)

Chunk 只会分裂，不会合并

## Caffeine

强大的高性能本地缓存框架，除了基本缓存功能，还提供了过期、异步加载、事件通知等功能

### 缓存淘汰算法

内存空间有限且昂贵，需要不断淘汰旧数据以保证新数据的空间，好的缓存淘汰算法：

- 缓存命中率高
- 维护成本低，即算法开销低
- 能够应对稀疏流量的场景，即缓存的目的
  - 稀疏流量指一个时间段内，流量大幅增加但不持续，短时间内恢复正常水平，如网站活动开启前后会有大量流量涌入

常用的缓存淘汰算法：

- FIFO：实现简单，缓存命中率较低
- LRU：实现简单，缓存命中率可以接受，但无法反映缓存使用频率，导致无法处理突发流量，在短时间内涌入大量流量时会加入大量新缓存，导致之前访问频率较高的被淘汰
  - LinkedHashMap 的 accessOrder 实现简单的内存 LRU
- LFU：能够反映缓存访问频率，占用内存大，无法处理稀疏流量
  - 内存开销：LFU 需要维护所有缓存项一定时间内的访问频率，每次访问都会更新相应的统计信息
  - 稀疏流量：热点数据访问频率变化较快，可能导致一些数据开始访问频率极高，之后不再被访问也会被一直保留，而会被频繁访问的新数据则无法与其竞争
- Clock：将缓存项看作循环链表，每个缓存项都有一个引用位，初始值为 0，被使用时置为 1，需要淘汰时扫描所有引用为 0 的缓存项，无法反映访问频率，且对访问模式敏感

Caffeine 的缓存淘汰算法：使用优化后的 LFU + FRU，即 W-TinyLFU

- Count-Min Sketch：大幅减小内存开销，存在误差但不影响 LFU 的使用(不需要精确的频率)

  - 基础 CMS：类似布隆过滤器，使用多个一维数组分别对应多个哈希函数，数组中的每个元素表示一个计数器

    - 当缓存项被访问时，每个哈希函数都对缓存项的 key 进行运算并将对应的哈希值计数器 +1
    - 获取频率时，计算 key 的所有哈希值，取各个数组中计数器的最小值作为频率

    - ![频率为最小值 1](https://img-blog.csdnimg.cn/20200919112318270.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L2xfZG9uZ3lhbmc=,size_16,color_FFFFFF,t_70#pic_center)

  - Caffeine 优化：

    - 使用一维数组(long[])代替二维数组
    - 缩小存储数值的大小：只使用 0-15 表示访问频率，即 4 bit 表示一个计数器，则一个 64 bit 的 long 类型能够存储 16 个计数器
    - 衰退减半：所有计数器的和超出阈值时，将所有计数器值除以 2，削弱频率较高的旧数据，保持缓存频率统计的时效性
    - ![](https://upload-images.jianshu.io/upload_images/6404076-a380cc85b645bf9e.png?imageMogr2/auto-orient/strip|imageView2/2/format/webp)

- Window Cache 处理稀疏流量：将缓存分为 3 个区域，使用 Protected 保存访问频率高的缓冲项

  - Window Cache：新的缓存项都会写入 Window Cache
  - Probation：Window Cache 写满后会写入 Probation
    - Probation 已满时会执行淘汰机制，类似 JVM 的分代回收，Probation 中访问频率较高的缓存项会被提升至 Protected
    - Protected 已满时，取 Probation 中频率最低的为 Victim，Window 及 Protected 中将移入 Probation 的为 Candidate，选择两者中频率较低的一方淘汰
  - Protected：Window Cache 中访问频率较高的写入 Protected
  - ![](https://upload-images.jianshu.io/upload_images/6404076-227c3535115ecdfa.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)

### 读写缓冲

缓存状态更新等操作一般随着读写操作一起执行，故缓存一般会使用同步加锁的方式进行处理，锁竞争的开销较大

Caffeine 中使用类似关系型数据库的 WAL(Write Ahead Logging)方案，通过缓冲区存储操作日志，之后再异步批量更新缓存

#### 读缓冲

数据本身在内部的 ConcurrentHashMap 中直接返回，数据读取产生的状态变动信息存入环形缓冲区中，由后台线程异步处理

每个线程都有独立的 RingBuffer(固定尺寸，头尾相连的缓冲区)，通过对线程取哈希值进行分配，由此进一步减少锁竞争

当线程处理速度小于 RingBuffer 填充速度时，会导致部分读信息的状态变更丢失

#### 写缓冲

读信息的丢失可以接受，但写操作则不能丢失

写入数据时，状态变更信息==使用阻塞队列存放==

### 异步数据清理

为了提高读写操作并发效率，Caffeine 采用异步方式处理数据淘汰与清理操作，但异步的延迟可能导致缓存大小短暂地超出限制

1. 缓存空间几乎已满
2. 多个线程写入
3. Caffeine 执行异步清理，尝试释放空间
4. 线程写入完毕，清理还未完成，此时缓存空间超出限制
5. 异步清理执行完毕，缓存空间回到合法大小

超出限制的时间一般十分短暂，对于其带来的高性能是可以接受的

### 时间轮

Caffeine 中使用时间轮实现过期策略，时间轮的写入与读取都为 O(1)，适用于任务数量多的定时任务

时间轮为环形队列，底层一般为数组，类比时钟，数组中的每个元素表示一个时刻，如精度为 1s 的时间轮中每个元素表示 1s，即 t[0] - t[10] 表示经过 10s

![](https://img-blog.csdnimg.cn/354490c0720246628f577a3d70d7a288.png)

为了表示更长的时间，可以引入轮数的概念表示下个任务应该经过 n 轮后才执行，还可以引入多层时间轮，如秒轮 → 分轮 → 时论，逐层

![](https://img-blog.csdnimg.cn/ebf7615e873342cc9086ce622a8201f4.png)

## Elasticsearch

开源、==分布式、RESTful 风格的搜索和分析引擎，用于解决数据库对海量数据进行模糊查询时的性能问题==，且支持所有数据类型

使用 Java 开发，基于 Lucene → 高性能的搜索引擎，但难以使用、扩展，且不支持分布式与高可用，Elasticsearch 封装 Lucene，自带分布式与高可用的解决方案，提供RESTful 风格的 API 调用

### 基本概念

- index：一组具有相同特征的文档集合，类似数据库表
- document：数据存储的最小单位，一般为 JSON 格式(非结构化)
- type：每个文档都必须设置类型，ES 7.0 前一个 index 可以有多个 type，ES 7.0 后一个 index 只能有一个 type，ES 8.0 后 type 被完全删除
- mapping：定义字段名称、数据类型、优化信息、分词器，类似数据库的表结构，一个 index 对应一个 mapping
  - mapping 定义后只能新增，不能修改，否则只能通过 reindex 重新索引或重新进行数据建模与导入数据
- node：一个 Elasticsearch 服务实例
- cluster：多个 node 的集合，解决单个节点无法处理的搜索与存储需求
- shard：index 分散存储在不同节点，提高性能和吞吐量
- replica：每个 index 有 1 到多个副本，用于提高扩展功能与吞吐量
- DSL：基于 JSON 的查询语言，类似数据库 SQL

### 倒排索引

由 文档 → 搜索词 的搜索方向变为 搜索词 → 文档

通过事先将文档进行分词，将分词与包含该分词的文档 id 相关联，由此可以快速查找包含该分词的文档，是一种空间换时间的数据结构，占用大量空间以提高检索效率

![](https://segmentfault.com/img/remote/1460000037659003)

- document：搜索的目标数据，每条数据都是一个文档
- term：基于分词算法得到的关键词
- dictionary：关键词的词典，顺序存储所有关键词，可以通过二分法快速查找，由 B+ 树或 Hash 拉链法存储，提高查询效率
  - 由于 ES 默认会为所有字段建立索引，dictionary 占用空间较大，直接使用会消耗大量内存空间
  - term index 是一种类似字典树的结构，用于在字符串集合中快速找到目标字符串，通过 term index 能够快速定位 dictionary 的某个 offset，再从该位置开始顺序查找(类似通过首字母查找字典)
  - term index 在内存中以 FST(Finite State Transducers) 保存

- posting list：记录关键词对应的文档集合
  - DocId：文档 id
  - TF：词频
  - Position：单词在文档出现的位置，用于检索
  - Offset：偏移量，记录单词开始结束位置

倒排索引建立流程：

1. 文档 → 文档 id
2. 分词器对文档进行分词，生成 <关键词，文档 id> 的一组数据
3. 将关键词作为索引关键字，记录词语和文档的对应关系，即 关键词 → 文档 id 列表
...
- 正排索引：
  - 维护成本低，新增数据时只需要在末尾新增 id
  - 以 DocId 为索引，每次都需要全量查询比较，效率低
- 倒排索引：
  - 查询效率高
  - 维护倒排索引的成本高，每次文档更新都需要重新建立倒排索引，且分词器精度会影响搜索精度(dog 与 dogs)

Elasticsearch 中不做索引的关键字无法被用于搜索

### 分词器

分词器为搜索引擎的核心，主要用于将 文档 → 词典

中文分词器：

- IK：常用的开源中文分词器
  - 智能模式：最粗粒度的分词，已经被分出的词语不会再进一步拆分
  - 细粒度切分模式：最细粒度的拆分，若分出的词语还能拆分，则会一直拆分到不可再分为止
- 拼音分词器：用于汉字和拼音之间的转换，实现基于拼音搜索

#### 组成部分

- Character Filters：对原始数据进行简单清洗，如去除 HTML 标签
- Tokenizer：实际进行分词的组件，按照规则对文本进行分词
- Token Filters：处理分词结果，大小写转换、切除停用词、添加近义词等

### 数据类型

#### 关键词 keyword vs 非结构化文本 text

keyword 为关键词，检索时不会进行分词处理

text 为文本，需要经过分词处理后进行检索
...
上文为一个 mapping 对名为 name 的字段结构的定义：

- 数据类型为 text，ES 对 text 分词以进行全文索引与相关性评分
- 包含子结构 fields
  - fields 中包含名为 keyword 的数据类型为 keyword 的字段，能够用于排序、聚合、精确匹配

通过在 text 字段内定义 keyword 字段，令 text 字段能够适用于全文搜索和聚合操作

#### nested 数据类型

数据扁平化：原本相关联的数据由于被拆分后分别存储，导致该关联性丢失

如：John White 与 Alice Smith 分别在 firstName 与 lastName 中存储
...
此时查询 John Smith，由于分别从 firstName 与 lastName 中都能查找到，故原本不存在的 John Smith 也能被检索

==nested 数据类型通过允许对象数组以一种可以相互独立查询的方式进行索引==，保持数组中对象的独立性

### Mapping

mapping 类似数据库的表结构，用于定义字段的数据类型、名称、优化信息、分词器，一个 index 对应一个 mapping

- 动态 mapping：根据文档结构自动生成 mapping 与 index
  - 属性配置：
    - true：新字段被添加到 mapping 中
    - runtime：新字段作为运行时字段添加到 mapping 这些字段未被编入索引，在 _source 查询时加载
    - false：新字段被忽略，字段不会被索引或搜索
    - strict：新字段必须显示添加到 mapping 中，否则报错
  - 通过映射限制设置来避免动态 mapping 中字段无限增加
- 显示 mapping：手动设置字段的存储和索引方式
  - 纯手写 mapping 工作量过大，可以创建临时 index，在其动态 mapping 基础上进行修改

#### 禁止字段被索引

通过在 mapping 中设置 index = false 令字段不可作为检索条件，但结果中仍会包含该字段

通过 index_options 控制倒排索引记录内容：
...
{
	"query": "SELECT * FROM unit ORDER BY itemid DESC LIMIT 5"
}
```

#### term vs 全文检索

- term 查询不会进行分词处理，一般用于非文本字段或精确查询
- 全文检索会进行分词处理，一般用于文本字段
  - 分词 → 词项逐个查询 → 计算关联性得分

#### 范围查询

range 查询用于匹配某一范围内的数值型、字符串型、日期类型字段的文档，range 只能查询一个字段，不能用在多个字段上
...
- match：多个检索词之间默认为 or，可由 operator 修改为 and
- match_phrase：多个检索词之间默认为 and，且检索词顺序会影响查询结果

#### multi match

用于单字段多条件查询：

- best_fields：查询结果包含任一查询条件，最终得分为最佳匹配字段得分
- most_fields：查询结果包含任一查询条件，最终得分合并所有匹配字段得分，查询条件默认为 or
- cross_fields：跨字段匹配，解决 most_fields 查询词无法使用 and 连接的问题，匹配更加精确
...
bool 一般用于多条件多字段查询，可以包含 match，match_phrase，term 等简单查询语句

- must：结果必须匹配所有查询条件
- should：结果至少匹配查询条件中的一个
- must_not：结果必须不匹配所有查询条件
- filter：结果必须匹配过滤条件，但不计算得分，提高查询效率

### 数据同步

#### 与 MySQL 同步

- 全量同步：建好 ES 索引后一次性导入所有 MySQL 数据
- 增量同步：将 MySQL 中的更改同步至 ES
  - 同步双写：写操作同时写入 MySQL 与 ES
    - 优点：实现简单
    - 缺点：
      - 业务耦合，负责写操作的业务与数据同步耦合
      - 性能低下
  - 异步双写：引入消息队列，写操作将变更放入消息队列，由搜索服务订阅并同步
    - 优点：
      - 解耦合，写操作业务只需要将变更放入消息队列，不需要关注实现
      - 实时性，使用 MQ 正常情况下同步完成在秒级
    - 缺点：
      - 引入额外组件，提升系统复杂性与维护成本
      - 在 ES 的聚合数据需要回查，即搜索服务直接查询数据库或远程调用
  - 定时服务：ES 定时查询 MySQL 进行同步，适用于数据量较小的场景
    - 优点：实现简单
    - 缺点：
      - 定时频率难以决定，无法保证实时性
      - 业务服务与搜索服务都直接访问数据库，对数据库的压力大
  - 数据订阅：类似 MySQL 的主从同步，借助 binlog 完成数据同步，可以通过第三方(canal)实现零代码，但需要聚合数据的场景仍需要回查
    - 优点：
      - 降低耦合度
      - 实时性
...
Canal 伪装成 MySQL 从库，以主从同步的方式实现增量同步

- Canal 向 MySQL 发起 binlog 请求
- MySQL 根据偏移量返回新的 binlog
- Canal 解析 binlog 并将数据同步至 ES

### Elasticsearch 集群

集群由多个节点组成，节点之间相互通信、配合，每个节点负责至少一个角色：

- 主节点：负责索引的新增/删除，跟踪哪些节点是集群的一部分，决定分片应分配给哪些节点
  - 专用备选主节点：主节点的备份，在主节点崩溃时保证服务正常
  - 仅投票主节点：用于在备选节点中选举出主节点，自身不参与选举
- 数据节点：负责数据存储、处理(CRUD、搜索、聚合)
- 预处理节点：执行由预处理管道组成的预处理任务
- 仅协调节点：路由分发请求，聚集搜索或聚合结果
- 远程节点：跨集群检索或跨集群复制

#### 分片

分片是集群存储数据的容器，索引被拆分为多个文档存储在分片中，分片被分配到集群中的节点，每个分片都是一个 Lucene 实例，能够对数据子集进行索引与检索

ES 集群的核心是对所有分片执行分布存储、索引、负载、路由的工作

集群扩容/缩容时，分片也会被重新分配，尽可能地保证数据的均匀分布

主分片负责写操作，副分片负责备份与提供读服务，主分片崩溃时从副分片中选举出新的主分片

#### 分片检索

查询数据需要先通过路由公式找到所在的分片：

- 请求到达仅协调节点
- 节点通过路由公式，hash(key) % num_of_shards，即对数据标识(一般为 docId)进行哈希计算后取余于分片总数，得到所在分片
- 将请求转发至分片所在的主节点

由于路由公式需要分片总数，故在集群创建前就应该确定分片总数，且在集群扩容/缩容时需要重新分配分片

#### 自定义路由

默认的路由公式会令数据尽量均匀分布，避免数据集中在一个分片，该分片崩溃则导致所有数据不可用

均匀分布的分片能保证稳定性，但也会令请求需要频繁转发，再将分散查询的结果聚合返回，在数据量较小的场景效率较低(数据分布太稀疏)

可以通过 API 中的 routing 参数自定义文档到数据分片的映射规则

#### 集群健康状态

- GREEN：主副分片都可用
- YELLOW：主分片可用，但没有可用的副分片
- RED：主分片不可用

### Elasticsearch 优化

硬件：Elasticsearch 的性能瓶颈在内存与磁盘

- 内存：操作需要占用大量内存，内存不足时会通过 swap 借用磁盘空间，导致效率下降
- 磁盘：海量的数据存储于磁盘

#### 索引优化

- Bulk API 进行批量写任务
  - 需要控制每个请求的大小，过大会占用大量内存
- 合理分配副分片，副分片的同步操作需要额外开销
- ES 使用延迟写入策略，默认 1s 后将内存中 segment 数据写回磁盘后才能将数据搜索出来(近实时搜索)，增加刷新间隔以减少磁盘 IO，提高写入索引的效率(每次写入更多)
- 增大 index_buffer_size，即所有分片共享的内存区，数据先写入 segment 后才被写回磁盘
- 使用 ES 默认的 ID 生成策略或使用数值型 id 作为主键
- 合理配置 index 属性：
  - analyzed/not_analyzed：根据业务需求决定是否分词

#### 查询优化

- 冷热分离，预热数据
- 自定义路由规则，将同一类型的文档存储至同一分片，减少路由转发
- copy_to 合并字段
- 控制字段数量，业务不使用的字段不创建索引
- 使用 _source 指定返回字段，不返回无用字段
- 控制文档大小，避免存储大文档
- keyword 数据类型避免分词
- 慢查询配置定位慢查询
- 过滤查询较多时，增大 querycache 以提高查询速度
- 避免深分页
- 增加副分片以提高读效率
